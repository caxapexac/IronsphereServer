...
class unit_prototype : public stts::parameter_map, public ityped {
    ...
    template<typename T>
    T ent::unit_prototype::get_parameter (const std::string& parameter_name, int ttl) {
    ...
    template<typename T>
    void ent::unit_prototype::set_parameter (const std::string& parameter_name, T data, int ttl) {
        if (--ttl <= 0) throw todo_exception("I don't know how but you have unit with 128 layers of inheritance");
        // TODO is needed?
