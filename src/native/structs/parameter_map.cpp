...
bool parameter_map::get (const std::string& name, vector2<int>& result) {
    auto iter = parameters_vector.find(name);
    if (iter != parameters_vector.end()) {
        result = iter->second;
        return true;
    }
    return false;
...
void parameter_map::set (const std::string& name, const vector2<int>& data) {
    parameters_vector[name] = data;
}

void parameter_map::set_path (std::queue<vector2<int>> data) {
    parameter_path = std::move(data);
}
